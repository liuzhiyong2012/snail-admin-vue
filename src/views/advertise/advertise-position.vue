<template>
  <div class="front-user-list-ctn app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="行数" prop="param" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.row||'--' }}</span>
        </template>
      </el-table-column>
      <el-table-column label="列数" prop="param" align="center">
        <template slot-scope="{ row }">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="行数" prop="row"><el-input v-model="temp.row" /></el-form-item>
        <el-form-item label="列数" prop="column"><el-input v-model="temp.column" /></el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        row: [{ required: true, message: '行数是必须的', trigger: 'change' }],
        column: [{ required: true, message: '列数必须的', trigger: 'change' }],
  ...
  methods: {
    ...
    getList() {
      ...
      AdvertisePositionApi.getAdvertisePosition(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          AdvertisePositionApi.insertAdvertisePosition(tempData).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          AdvertisePositionApi.updateAdvertisePosition(tempData).then(response => {
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
              message: '成功更新一条记录',
    ...
    handleDelete(row, index) {
      this.listLoading = true
      AdvertisePositionApi.deleteAdvertisePosition(row).then(response => {
        this.$notify({
          title: '成功',
          message: '成功删除一条记录',
          type: 'success',
